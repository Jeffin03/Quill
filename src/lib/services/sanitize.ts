/* ══════════════════════════════════════════
   Quill — Request Sanitization
   Ported from docs/js/sanitize.js.
   Replaces terms that trigger guardrails on
   gated APIs (OpenRouter, NIM) with placeholders,
   then restores them in the response.
   ══════════════════════════════════════════ */

const RULES: { pattern: RegExp; placeholder: string }[] = [
	...
	{
		pattern: /\b(Dragon Ball|DBZ|Dragon Ball Z|Dragon Ball Super)\b/gi,
		placeholder: '[[SERIES_DBZ]]'
	},
	...
	{
		pattern: /\bexplicit\s+(sexual|content|scene|fanfic|fiction|writing)\b/gi,
		placeholder: 'mature'
	},
...
let _replacements: { placeholder: string; original: string }[] = [];
...
function _sanitizeText(text: string): string {
	...
	for (const rule of RULES) {
		result = result.replace(rule.pattern, (match) => {
			_replacements.push({ placeholder: rule.placeholder, original: match });
...
function restoreText(text: string): string {
	let result = text;
	const unique = [...new Map(_replacements.map((r) => [r.placeholder, r.original])).entries()].sort(
...
function isGatedProvider(provider: string): boolean {
	return provider === 'openrouter' || provider === 'nim';
}

export { RULES, reset, sanitizeMessages, restoreText, isGuardrailRefusal, isGatedProvider };
